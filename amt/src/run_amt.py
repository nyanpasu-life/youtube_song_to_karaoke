...
def transcribe(model, audio_info):
    ...
    # Write MIDI
    write_model_output_as_midi(pred_notes, '../../tmp/',
                              audio_info['track_name'], model.midi_output_inverse_vocab)
    t.stop(); t.print_elapsed_time("post processing");
    midifile =  os.path.join('../../tmp/model_output/', audio_info['track_name']  + '.mid')
...
if __name__=="__main__":
    ...
    for root, dirs, files in os.walk("../../tmp/vocal"):
        for f in files:
            if f.endswith(".mp3"):

                file_path = os.path.abspath(os.path.join(root, f))
                track_name = Path(f).stem

                audio_info = {"filepath": file_path, "track_name": track_name}
